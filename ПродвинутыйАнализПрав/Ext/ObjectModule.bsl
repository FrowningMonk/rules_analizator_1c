...
Функция ПолучитьИтоговуюТаблицу(Параметры) Экспорт
	...
	ОбъектыМетаданных = Неопределено;  // Массив
	ИменаРолей = Неопределено;          // Массив
	Профиль = Неопределено;
	ФильтрТипДоступа = Неопределено;
	ПоказыватьПрофили = Истина;
	ФильтрОбъектов = Неопределено;

	// Поддержка массива объектов (новый формат)
	Если Параметры.Свойство("ОбъектыМетаданных") Тогда
		ОбъектыМетаданных = Параметры.ОбъектыМетаданных;
	// Обратная совместимость - одиночное значение
	ИначеЕсли Параметры.Свойство("ОбъектМетаданных") И ЗначениеЗаполнено(Параметры.ОбъектМетаданных) Тогда
		ОбъектыМетаданных = Новый Массив;
		ОбъектыМетаданных.Добавить(Параметры.ОбъектМетаданных);
	Иначе
		ОбъектыМетаданных = Неопределено;
	КонецЕсли;

	// Поддержка массива ролей (новый формат)
	Если Параметры.Свойство("ИменаРолей") Тогда
		ИменаРолей = Параметры.ИменаРолей;
	// Обратная совместимость - одиночное значение
	ИначеЕсли Параметры.Свойство("ИмяРоли") И ЗначениеЗаполнено(Параметры.ИмяРоли) Тогда
		ИменаРолей = Новый Массив;
		ИменаРолей.Добавить(Параметры.ИмяРоли);
	Иначе
	...
	Если ОбъектыМетаданных <> Неопределено И ОбъектыМетаданных.Количество() > 0 Тогда
		// Режим: анализ по объектам метаданных (множественный)
		Для Каждого ПолноеИмяОбъекта Из ОбъектыМетаданных Цикл
			АнализПоОбъектуМетаданных(РезультатАнализа, ПолноеИмяОбъекта, ПоказыватьПрофили,
				ФильтрТипДоступа, Профиль, ИменаРолей);
		КонецЦикла;

	ИначеЕсли ЗначениеЗаполнено(Профиль) Тогда
		// Режим: анализ по профилю (с возможной фильтрацией по ролям)
		АнализПоПрофилю(РезультатАнализа, Профиль, ФильтрТипДоступа, ФильтрОбъектов, ИменаРолей);

	ИначеЕсли ИменаРолей <> Неопределено И ИменаРолей.Количество() > 0 Тогда
		// Режим: анализ по ролям (множественный)
...
//   ФильтрИменаРолей - Массив, Неопределено - фильтр по именам ролей (множественный выбор)
//
Процедура АнализПоОбъектуМетаданных(Результат, ПолноеИмяОбъекта, ПоказыватьПрофили,
	ФильтрТипДоступа, ФильтрПрофиль = Неопределено, ФильтрИменаРолей = Неопределено)
	...
	Для Каждого СтрокаРоли Из ОписаниеРолей Цикл
		...
		// Фильтрация по массиву имён ролей (если задан)
		Если ФильтрИменаРолей <> Неопределено И ФильтрИменаРолей.Количество() > 0 Тогда
			Если ФильтрИменаРолей.Найти(СтрокаРоли.ИмяРоли) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
//   ФильтрИменаРолей - Массив, Неопределено - фильтр по именам ролей (роли должны входить в профиль)
//
Процедура АнализПоПрофилю(Результат, Профиль, ФильтрТипДоступа, ФильтрОбъектов = Неопределено,
	ФильтрИменаРолей = Неопределено)
	...
	// Фильтрация по массиву ролей если задано
	Если ФильтрИменаРолей <> Неопределено И ФильтрИменаРолей.Количество() > 0 Тогда
		ОтфильтрованныеРоли = Новый Массив;
		Для Каждого ОписаниеРоли Из РолиПрофиля Цикл
			Если ФильтрИменаРолей.Найти(ОписаниеРоли.ИмяРоли) <> Неопределено Тогда
