...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СистемныйПромптИИ = "Ты - опытный аналитик 1С. Твоя задача - проанализировать доступы к объектам системы 1С. "
		+ "Кратко расскажи про технические объекты, которые влияют на формы, макеты, штатную пользовательскую работу. "
		+ "Формат ответа: емкое резюме на 3-4 предложения. ";
...
// Обработчик начала выбора для поля "Имя роли"
// Открывает форму выбора объектов метаданных с фильтром по ролям
//
&НаКлиенте
Процедура ИмяРолиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;

	ПараметрыФормы = СтандартныеПодсистемыКлиентСервер.ПараметрыВыбораОбъектовМетаданных();
	// Множественный выбор включен по умолчанию (ВыборЕдинственного = Ложь)

	// Фильтр только на роли
	ПараметрыФормы.КоллекцииВыбираемыхОбъектовМетаданных.Добавить("Роли");

	// Передать уже выбранные для отметки в дереве (ВыбранныеОбъектыМетаданных - СписокЗначений)
	Для Каждого ЭлементСписка Из ИмяРоли Цикл
		ПараметрыФормы.ВыбранныеОбъектыМетаданных.Добавить("Роль." + ЭлементСписка.Значение);
	КонецЦикла;

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ИмяРолиВыборЗавершение", ЭтотОбъект);
...
// Обработчик завершения выбора ролей (множественный)
//
&НаКлиенте
Процедура ИмяРолиВыборЗавершение(ВыбранныеОбъекты, ДополнительныеПараметры) Экспорт

	Если ВыбранныеОбъекты = Неопределено
		Или ВыбранныеОбъекты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ВыбранныйОбъект Из ВыбранныеОбъекты Цикл
		// Формат: "Роль.ИмяРоли" - извлекаем только имя
		ПолноеИмя = ВыбранныйОбъект.Значение;
		ЧастиИмени = СтрРазделить(ПолноеИмя, ".");
		Если ЧастиИмени.Количество() >= 2 Тогда
			ИмяРоли.Добавить(ЧастиИмени[1]);
		КонецЕсли;
	КонецЦикла;

КонецПроцедуры

// Обработчик начала выбора для поля "Объект метаданных"
// Открывает форму выбора объектов метаданных с поддержкой множественного выбора
//
&НаКлиенте
Процедура ОбъектМетаданныхНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// Передать уже выбранные для отметки в дереве (ВыбранныеОбъектыМетаданных - СписокЗначений)
	Для Каждого ЭлементСписка Из ОбъектМетаданных Цикл
		ПараметрыФормы.ВыбранныеОбъектыМетаданных.Добавить(ЭлементСписка.Значение);
	КонецЦикла;

	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ОбъектМетаданныхВыборЗавершение", ЭтотОбъект);
...
// Обработчик завершения выбора объектов метаданных (множественный)
//
&НаКлиенте
Процедура ОбъектМетаданныхВыборЗавершение(ВыбранныеОбъекты, ДополнительныеПараметры) Экспорт
	...
	ОбъектМетаданных.Очистить();

	Для Каждого ВыбранныйОбъект Из ВыбранныеОбъекты Цикл
		// Полное имя: "Справочник.Номенклатура"
		ОбъектМетаданных.Добавить(ВыбранныйОбъект.Значение);
	КонецЦикла;
...
Процедура ПолучитьТЗНаСервере()
	...
	// Передача массивов объектов метаданных
	Если ОбъектМетаданных.Количество() > 0 Тогда
		ПараметрыАнализа.Вставить("ОбъектыМетаданных", ОбъектМетаданных.ВыгрузитьЗначения());
	КонецЕсли;

	// Передача массива ролей
	Если ИмяРоли.Количество() > 0 Тогда
		ПараметрыАнализа.Вставить("ИменаРолей", ИмяРоли.ВыгрузитьЗначения());
	КонецЕсли;
...
Процедура АналитикаПравНаСервере(ТекстПрав)
	...
	Если Не Ответ.ЭтоОшибка Тогда
		ТекстРазметки = КИИ_МаркдаунПарсерКлиентСервер.ПреобразоватьВHTML(Ответ.ТекстОтвета);
		ОтветИИHTML = СформироватьРазметкуОтвета(
			ТекстРазметки, Ответ.ВсегоТокенов, Ответ.ВходныеТокены, Ответ.ВыходныеТокены);
	Иначе
		ОтветИИHTML = СформироватьРазметкуОшибки(Ответ.ТекстОтвета);
	КонецЕсли;

КонецПроцедуры

&НаСервере
Функция СформироватьРазметкуОтвета(ТекстРазметки, ВсегоТокенов, ВходныеТокены, ВыходныеТокены)

	Возврат "<html><body style=""font-family: Arial, sans-serif; padding: 10px;"">"
		+ ТекстРазметки
		+ "<hr/><p style=""color: gray; font-size: 12px;"">Использовано всего токенов: "
		+ Формат(ВсегоТокенов, "ЧГ=0") + ". Из них на вход: "
		+ Формат(ВходныеТокены, "ЧГ=0") + ". На выход: "
		+ Формат(ВыходныеТокены, "ЧГ=0") + ".</p>"
		+ "</body></html>";

КонецФункции

&НаСервере
Функция СформироватьРазметкуОшибки(ТекстОшибки)

	Возврат "<html><body style=""font-family: Arial, sans-serif; padding: 10px; color: red;"">"
		+ "<p>Ошибка: " + КИИ_МаркдаунПарсерКлиентСервер.ЭкранироватьHTML(ТекстОшибки) + "</p>"
		+ "</body></html>";
