...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВключатьРегистрыНакопления = Ложь;
...
Процедура АналитикаПравНаСервере(ТекстПрав)
	...
	Если Не Ответ.ЭтоОшибка Тогда
		...
		ОтветИИHTML = СформироватьРазметкуОтвета(ТекстРазметки, Ответ.ВсегоТокенов);
	Иначе
...
&НаСервере
Функция СформироватьРазметкуОтвета(ТекстРазметки, ВсегоТокенов)

	Возврат "<html><body style=""font-family: Arial, sans-serif; padding: 10px;"">"
		+ ТекстРазметки
		+ "<hr/><p style=""color: gray; font-size: 12px;"">Использовано токенов: "
		+ Формат(ВсегоТокенов, "ЧГ=0") + "</p>"
		+ "</body></html>";
